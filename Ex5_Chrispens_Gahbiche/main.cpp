...
void futurePormise(const int startValue, const std::size_t nMax) {
    ...
    std::vector<std::promise<int>> transportP1(nMax); // assumes granularity 1
    std::vector<std::promise<std::array<int, 3>>> transportP2(nMax);

    //auto transportP3 = std::make_shared<MulticastMutexTransport<std::tuple<std::set<int>, std::set<int> >>>();
    //auto sourceIdP3 = transportP3->addSource();
    std::vector<std::promise<std::tuple<std::set<int>, std::set<int> >>> transportP3 (nMax);
    ...
    std::thread([&] {
        ...
        transportP1[0].set_value(startValue);

        for (std::size_t n = 1; n <= nMax; ++n) { //run through the count we set in start
            if (prevCollatzValue % 2 == 0) {
                nextCollatzValue = prevCollatzValue / 2;
                transportP1[n].set_value(nextCollatzValue);
            } else {
    ...
    std::thread([&] {
        ...
        std::set<int> ArrayOfSubstractions;
        // consume produced elements
        for (size_t n = 0; n < transportP1.size(); n++) {
            auto next = transportP1[n].get_future().get();
    ...
    std::thread([&] {
        ...
        for(size_t n=0; n<transportP2.size(); n++){

            auto next = transportP2[n].get_future().get();
            ++counter_n;
            auto a_n =   next[0];
            auto min_n = next[1];
            auto max_n = next[2];
    ...
    // consume produced elements
    //alternatively just joun() producer 3 instead of detach!
    for(size_t n=0; n<transportP3.size(); n++){
        auto next = transportP3[n].get_future().get();
        for(auto m0 : std::get<0>(next)){
            std::cout << "Final Result m0: " << m0 << std::endl;
        }
        for(auto m1 : std::get<1>(next)){
